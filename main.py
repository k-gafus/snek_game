...
BODY_SIZE = 3 # Not yet implemented in all related locations
...
class Snek_Control:
    def __init__(self, starting_x=0, starting_y=0, max_x=128, max_y=64):
        ...
        # Testing a different logic for direction control on seperate compiler
        self.vector_dict = {
        "left": (-3, 0),
        "right": (3, 0),
        "up": (0, -3),
    ...
    def move_x_left(self):
        if self.dx == 3: # Logic for restricting movement on same axis
            ...
            self.dx = -3 # Note to self, Pythonic way would be: self.dx, self.dy = -3, 0
            self.dy = 0
...
#######################
# ---- Functions ---- # Need to eventually purge or implement unused funtions
#######################
...
def display_update():
    display_obj.fill(0)
    display_obj.text("The Snek v1.0", 20, 32, 1)
    display_obj.show()
...
def get_joystick_event():
    raw_x = joy_x.read_u16() - 32759 # To offset joystick values (center 0)
...
# ---- Class Objects ---- #
###########################
snek_control = Snek_Control(starting_x=10, starting_y=32)
